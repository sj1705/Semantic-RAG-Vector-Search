...
def reports_to_markdown(reports: Iterable[QueryReport]) -> str:
    ...
    lines.append("")
    lines.append(
        "This report is generated by `scripts/run_benchmark.py`. It compares three "
        "retrieval strategies over the corpus in `data/corpus.py`:"
    )
    lines.append("")
    lines.append("- **Strategy A** — raw embedding similarity (baseline).")
    lines.append(
        "- **Strategy B (rewrite)** — a mocked `GenerativeModel` rewrites the query "
        "with domain synonyms before embedding."
    )
    lines.append(
        "- **Strategy B (HyDE)** — the mock produces three hypothetical answer "
        "passages; their embeddings are averaged and re-normalized, then searched."
    )
    lines.append("")
    lines.append(
