...
def make_pipeline_factory_from_store(store, embedder):
    ...
    Used when the caller has already loaded or built the index (e.g. from
    ``FaissVectorStore.load``). The factory spins up a ``RAGPipeline`` that
    shares the exact same index across all 30 benchmark calls — zero
    re-embedding.
    """
